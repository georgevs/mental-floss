...
const maximumScore = (ns, ms) => {
  ...
  const iter = (i, j, n) => {
    const f = ms[i] * ns[j];
    const l = ms[i] * ns[j + n - 1];
    return (
      i + 1 == ms.length ? Math.max(f, l) :
      Math.max(
        f + dp(i + 1, j + 1, n - 1),
        l + dp(i + 1, j, n - 1),
      )
  ...
  return dp(0, 0, ns.length);
}

const memoize = (m, f) => (i, j, n) => (k => m.get(k) ?? m.set(k, f(i, j, n)))(hash(i, j, n));
const hash = (i, j, n) => [i, j, n].toString();
const map = (i) => { 
  const m = new Map(i);
