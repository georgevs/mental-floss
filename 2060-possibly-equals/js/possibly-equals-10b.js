...
const eq = ([{ ch: ch1, ns: ns1 }, ...r1], [{ ch: ch2, ns: ns2 }, ...r2], q) => {
  ...
  if (ch1 && ch2) {
    if (ch1 == ch2) if (eq(r1, r2)) return true;
    return false;
  } 
  ...
  if (ch1 && ns2) {
    const ns = dec(ns2, 1);
    return eq(r1, ns.length > 0 ? [{ ns }].concat(r2) : r2);
  }

  if (ns1 && ch2) {
    const ns = dec(ns1, 1);
    return eq(ns.length > 0 ? [{ ns }].concat(r1) : r1, r2);
  }
  ...
  for (const n of ns) {
    if (n > 0) { if (eq([{ ns: [n] }].concat(r1), r2)) return true }
    else if (n < 0) { if (eq(r1, [{ ns: [-n] }].concat(r2))) return true }
    else { if (eq(r1, r2)) return true }
  }
  return false;
};
