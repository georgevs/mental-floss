...
const eq = ([ch1, ns1, r1], [ch2, ns2, r2], q) => {
  ...
  if (ch1 && ch2) {
    if (ch1 == ch2) q.push([split(r1), split(r2)]);
    return false;
  } 
  ...
  if (ch1 && ns2) {
    const ns = dec(ns2, 1);
    q.push([split(r1), ns.length > 0 ? [undefined, ns, r2] : split(r2)]);
    return false;
  }

  if (ns1 && ch2) {
    const ns = dec(ns1, 1);
    q.push([ns.length > 0 ? [undefined, ns, r1] : split(r1), split(r2)]);
    return false;
  }
  ...
  // console.log(ns);
  for (const n of ns) {
    if (n > 0) { q.push([[undefined, [n], r1], split(r2)]) }
    else if (n < 0) { q.push([split(r1), [undefined, [-n], r2]]) }
    else { q.push([split(r1), split(r2)]) }
  }
  return false;
};
