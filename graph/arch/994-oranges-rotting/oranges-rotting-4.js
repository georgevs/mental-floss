...
const orangesRotting = (xs) => {
  ...
  while (q) {
    do {
      ...
      ([y, x, q] = q);
      r = l; ++m;
      if (0 < x && xs[y][x - 1] === 1) { xs[y][x - 1] = 2; nq = [y, x - 1, nq] }
      if (0 < y && xs[y - 1][x] === 1) { xs[y - 1][x] = 2; nq = [y - 1, x, nq] }
      if (x < X && xs[y][x + 1] === 1) { xs[y][x + 1] = 2; nq = [y, x + 1, nq] }
      if (y < Y && xs[y + 1][x] === 1) { xs[y + 1][x] = 2; nq = [y + 1, x, nq] }
